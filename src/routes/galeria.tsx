...
function GalleryFigure({src,alt,label,wide=false,parallax=false,onOpen}:{src:string;alt:string;label:string;wide?:boolean;parallax?:boolean;onOpen?:()=>void}){const parallaxRef=useParallax<HTMLImageElement>(0.08,30);return <figure className={cn("group overflow-hidden rounded-2xl border border-border transition-[transform,box-shadow] duration-300 hover:-translate-y-2 hover:shadow-xl",wide&&"md:col-span-2")}><button type="button" onClick={onOpen} aria-label={`Ampliar foto: ${label}`} className={cn("block w-full cursor-zoom-in",wide?"aspect-[4/3] sm:aspect-[16/9]":"aspect-[4/5]")}><img ref={parallax?parallaxRef:undefined} src={src} alt={alt} width={wide?1400:800} height={wide?788:1000} loading="lazy" className={cn("h-full w-full object-cover transition duration-700 group-hover:scale-[1.03]",parallax&&"scale-110 will-change-transform")}/></button><figcaption className="border-t border-border bg-background px-5 py-4 text-xs font-bold uppercase tracking-[0.14em] text-muted-foreground">{label}</figcaption></figure>}
function GalleryPage(){
  const [lightbox,setLightbox]=useState<{items:LightboxItem[];index:number}|null>(null);
  return <><PageHero eyebrow="Portfólio real" title="Nosso espaço. Nosso trabalho." description="Fotos reais da Fino Trato, organizadas entre o ambiente, resultados de clientes e publicações do Instagram."/><section className="py-14 sm:py-20"><div className="mx-auto max-w-7xl px-5 sm:px-8 lg:px-10"><Reveal><p className="eyebrow">A barbearia</p><h2 className="mt-3 font-display text-4xl font-semibold sm:text-5xl">Por dentro da Fino Trato.</h2></Reveal><div className="mt-9 grid gap-4 md:grid-cols-2">{spaces.map((item,index)=><Reveal key={item.label} delay={index*100}><GalleryFigure {...item} onOpen={()=>setLightbox({items:spaces,index})}/></Reveal>)}</div></div></section><section className="bg-muted py-14 sm:py-20"><div className="mx-auto max-w-7xl px-5 sm:px-8 lg:px-10"><Reveal><p className="eyebrow">Resultados reais</p><h2 className="mt-3 font-display text-4xl font-semibold sm:text-5xl">Cortes feitos por aqui.</h2></Reveal><div className="mt-9 grid grid-cols-2 gap-3 md:grid-cols-4 md:gap-4">{results.map((item,index)=><Reveal key={item.label} delay={index*80}><GalleryFigure {...item} onOpen={()=>setLightbox({items:results,index})}/></Reveal>)}</div></div></section><section className="py-14 sm:py-20"><div className="mx-auto max-w-7xl px-5 sm:px-8 lg:px-10"><Reveal><p className="eyebrow">@barbeariafinotrato</p><div className="flex flex-col gap-5 sm:flex-row sm:items-end sm:justify-between"><h2 className="mt-3 font-display text-4xl font-semibold sm:text-5xl">Do nosso Instagram.</h2><a href="https://www.instagram.com/barbeariafinotrato/" target="_blank" rel="noreferrer" className="text-sm font-bold text-primary hover:underline">Ver perfil oficial</a></div></Reveal><div className="mt-9 grid grid-cols-2 gap-3 md:grid-cols-3 md:gap-4">{instagramPosts.map((item,index)=><Reveal key={item.label} delay={index*80}><GalleryFigure {...item} onOpen={()=>setLightbox({items:instagramPosts,index})}/></Reveal>)}</div><div className="mt-14 flex justify-center"><Button asChild size="lg"><Link to="/agendamento">Quero renovar o visual <ArrowRight/></Link></Button></div></div></section>{lightbox&&<Lightbox items={lightbox.items} index={lightbox.index} onClose={()=>setLightbox(null)} onNavigate={(index)=>setLightbox((prev)=>prev?{...prev,index}:null)}/>}</>;
}